...
public class lightAttack : MonoBehaviour
{
    ...
    playerMovement move; // grabs the movement script 
    ...
    private void Update()
    {
        ...
        if (move.facing)
        {
            force = 40f; // this allows the attack to apply a amount of force and what direction the force sends the hit player 
        }
        if (!move.facing)
        {
            force = -40f;
    ...
    private void OnCollisionEnter(Collision col)
    {
        ...
        if (col.gameObject.tag == "Player" && col.gameObject != self) // if the attack hits a tagged object with the player tag and its not hitting itself it will apply the following affects 
        {
            ...
            ph.healthlite(); // deals the damage held within the attack
            playerEnergy pe = self.GetComponent<playerEnergy>();
            pe.Energylite(); // also gives amount of energy assigned to the attack
